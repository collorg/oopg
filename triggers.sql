...
CREATE FUNCTION before_insert_tuple() RETURNS trigger AS $$
oid = TD['relid']
fqtn = plpy.execute("""SELECT get_fqtn('%s')""" % oid)[0]['get_fqtn']
GD['td'] = TD
plpy.execute("SELECT check_uid('%s', '%s', '%s')" % (oid, fqtn, oid))
$$
LANGUAGE plpythonu;
...
CREATE FUNCTION before_update_tuple() RETURNS trigger AS $$
...
GD['td'] = TD
rec = plpy.execute("SELECT check_uid('%s', '%s', '%s')" % (oid, fqtn, oid))
$$
LANGUAGE plpythonu;
...
CREATE FUNCTION check_uid(integer, text, integer) RETURNS text AS $$
"""
...
parent_oid = plpy.execute(
    "SELECT get_inhparent('%s')" % oid)[0]['get_inhparent']
